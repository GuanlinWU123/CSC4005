...
Matrix matrix_multiply_mpi(const Matrix& matrix1, const Matrix& matrix2, size_t startRow, size_t endRow) {
    size_t M = matrix1.getRows(), K = matrix1.getCols(), N = matrix2.getCols();

    Matrix localResult(endRow - startRow, N);
    ...
    #pragma omp parallel
    {
        ...
        for (size_t i = startRow; i < endRow; i += TILE_SIZE) {
            for (size_t j = 0; j < N; j += TILE_SIZE) {
                ...
                // Copy the results from cTile to the localResult matrix
                for (size_t x = 0; x < TILE_SIZE && i + x < endRow; ++x) {
                    for (size_t y = 0; y < TILE_SIZE && j + y < N; ++y) {
                        localResult[i - startRow + x][j + y] = cTile[x][y];
                    }
                }
            }
        }
...
int main(int argc, char** argv) {
    ...
    size_t rowsPerProcess = M / numtasks;
    size_t startRow = taskid * rowsPerProcess;
    size_t endRow = (taskid == numtasks - 1) ? M : startRow + rowsPerProcess;

    Matrix localResult = matrix_multiply_mpi(matrix1, matrix2, startRow, endRow);

    if (taskid == MASTER) {
        Matrix result(M, matrix2.getCols());
        for (size_t i = 0; i < rowsPerProcess; i++) {
        ...
        for (int p = 1; p < numtasks; p++) {
            size_t start = p * rowsPerProcess;
            size_t end = (p == numtasks - 1) ? M : start + rowsPerProcess;
            for (size_t i = start; i < end; i++) {
                MPI_Recv(result[i], matrix2.getCols(), MPI_INT, p, 0, MPI_COMM_WORLD, &status);
            }
        }
        ...
        for (size_t i = 0; i < endRow - startRow; i++) {
            MPI_Send(localResult[i], matrix2.getCols(), MPI_INT, MASTER, 0, MPI_COMM_WORLD);
        }
    }
